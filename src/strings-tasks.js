...
function concatenateStrings(value1, value2) {
  const concatStrings = value1.concat(value2);
...
function removeLeadingAndTrailingWhitespaces(value) {
  const spacesRemoves = value.trim();
...
function removeLeadingWhitespaces(value) {
  const spacesRemoves = value.trimStart();
  return spacesRemoves;
}
...
function removeTrailingWhitespaces(value) {
  const spacesRemoves = value.trimEnd();
  return spacesRemoves;
}
...
function formatTime(minutes, seconds) {
  const minFormat = minutes.toString().padStart(2, '0');
...
function findLongestWord(sentence) {
  ...
  const newLongestWord = word.reduce((longestWord, currentWord) => {
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((wordStr) => wordStr.split('').reverse().join(''))
    .join(' ');
}
